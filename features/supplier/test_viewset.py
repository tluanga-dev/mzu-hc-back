...
class SupplierViewSetTestCase(APITestCase):
    def setUp(self):
        ...
        self.supplier = Supplier.objects.create(name='Test Supplier', contact=1234567890.0, email='test@example.com', address='Test Address', remarks='Test Remarks', is_active=True)
    ...
    def test_create_supplier(self):
        data = {'name': 'New Supplier', 'contact': 9876543210.0, 'email': 'new@example.com', 'address': 'New Address', 'remarks': 'New Remarks', 'is_active': True}
        response = self.client.post(self.url, data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_update_supplier(self):
        data = {'name': 'Updated Supplier', 'contact': 9876543210.0, 'email': 'updated@example.com', 'address': 'Updated Address', 'remarks': 'Updated Remarks', 'is_active': True}
