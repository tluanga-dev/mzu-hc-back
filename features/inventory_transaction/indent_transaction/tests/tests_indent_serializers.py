...
class IndentInventoryTransactionSerializerTestCase(BaseTestCase):
    ...
    def test_create_indent_inventory_transaction(self):
        ...
        serializer = IndentInventoryTransactionSerializer(
            data=self.indent_transaction_data)
        ...
        self.assertEqual(indent_transaction.inventory_transaction_type,
                         InventoryTransaction.TransactionTypes.INDENT)
        self.assertEqual(indent_transaction.supplier, self.supplier)
        self.assertEqual(indent_transaction.supply_order_no,
                         'SO1/992-223/222\2')

        transaction_items = InventoryTransactionItem.objects.filter(
            inventory_transaction=indent_transaction)
