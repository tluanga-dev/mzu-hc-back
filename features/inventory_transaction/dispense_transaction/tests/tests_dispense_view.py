...
class IndentInventoryTransactionViewSetTest(BaseTestCase):
    ...
    def test_create_indentinventorytransaction(self):
        ...
        url = reverse('dispense-inventory-transactions-list')
        ...
        #     'status': 'pending',
        # }
        response = self.client.post(url, self.dispense_transaction_data, format='json')
    ...
    #         'inventory_transaction_item_set': [
    #             {'item_batch': str(self.item_batch_2.id),
    #              'quantity': 10, 'is_active': True},
    #         ],
    #         'inventory_transaction_type': 'indent',
    ...
    #     )
    #     url = reverse('indent-inventory-transactions-detail',
    #                   kwargs={'pk': indent_transaction.pk})
    ...
    #     # Check that one instance is returned
    #     self.assertEqual(len(filtered_response.data), 1)
    #     # Check that the returned instance has the correct supplyOrderNo
    #     self.assertEqual(filtered_response.data[0]['supply_order_no'], 'SO12')
    ...
    #         'indent-inventory-transactions-list'), {'supply_order_date': '01-01-2022'})
    ...
    #     response = self.client.get(reverse('indent-inventory-transactions-list'), {
    #                                'supply_order_dateFrom': '01-02-2022', 'supply_order_dateTo': '01-03-2024'})
    #     self.assertEqual(response.status_code, status.HTTP_200_OK)
    #     self.assertEqual(len(response.data), 1)

    #     # Test filtering by a range of dates
    #     response = self.client.get(reverse('indent-inventory-transactions-list'), {
    #                                'supply_order_dateFrom': '01-01-2022', 'supply_order_dateTo': '01-04-2024'})
    #     self.assertEqual(response.status_code, status.HTTP_200_OK)
